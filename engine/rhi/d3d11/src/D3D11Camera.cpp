// ============================================================================
// D3D11Camera.cpp — 左手系视角相机实现
// 里程碑：M2
// 职责：实现相机的方位角/俯仰角处理、局部坐标移动与观察/投影矩阵构造；入参通过
//   ME_VERIFY 做有限值校验，保证朝向向量与矩阵始终有限。纯 CPU，不依赖 GPU。
...
D3D11Camera::D3D11Camera() = default;

void D3D11Camera::SetPosition(const float x, const float y, const float z) noexcept
{
...
void D3D11Camera::SetLens(const float verticalFovRadians, const float aspect, const float nearZ, const float farZ)
{
...
void D3D11Camera::SetAspect(const float aspect)
{
...
void D3D11Camera::AddYawPitch(const float yawDelta, const float pitchDelta) noexcept
{
...
void D3D11Camera::MoveLocal(const float right, const float up, const float forward) noexcept
{
...
DirectX::XMVECTOR D3D11Camera::PositionVector() const noexcept
{
    return DirectX::XMLoadFloat3(&m_position);
}

// 由 yaw/pitch 展开为球面坐标单位方向，即相机前向。
DirectX::XMVECTOR D3D11Camera::ForwardVector() const noexcept
{
...
DirectX::XMMATRIX D3D11Camera::ViewMatrix() const noexcept
{
...
DirectX::XMMATRIX D3D11Camera::ProjectionMatrix() const noexcept
{
    // 左手透视投影，使用最新 FOV/纵横比/近远平面。
    return DirectX::XMMatrixPerspectiveFovLH(m_verticalFovRadians, m_aspect, m_nearZ, m_farZ);
}

float D3D11Camera::Aspect() const noexcept
{
    return m_aspect;
}

float D3D11Camera::Yaw() const noexcept
{
    return m_yaw;
}

float D3D11Camera::Pitch() const noexcept
{
    return m_pitch;
}
} // namespace MiniEngine
