...
#
# 用法：
#   pwsh tools/compare_m7_results.ps1 -Baseline out/m7-05/serial -Candidate out/m7-05/parallel `
#        -TreatmentField packetBuildMode -OutputJson out/m7-09/compare.json
#   pwsh tools/compare_m7_results.ps1 -Baseline out/m7-08/sweep -BaselineFilter '*-aos' `
#        -Candidate out/m7-08/sweep -CandidateFilter '*-hot-cold' -TargetMetric layoutCullMs
#   pwsh tools/compare_m7_results.ps1 -Baseline <runs> -Candidate <runs> -SelfCheck
# ============================================================================
param(
...
# Portability only: load the matching PowerShell utility module.
...
foreach ($key in $baselineCells) {
    ...
    if ($a.hitch16_67 -gt 0) {
        $hitchDeltaPercent = 100.0 * ($b.hitch16_67 - $a.hitch16_67) / $a.hitch16_67
    ...
    $protectedDetail['hitch16_67'] = [ordered]@{ baseline = $a.hitch16_67; candidate = $b.hitch16_67; changePercent = [math]::Round($hitchDeltaPercent, 3); ok = $hitchOk }

    $drift = Get-Drift -BaselineCellRuns $aRuns -CandidateCellRuns $bRuns -RequiredPercent $a.requiredPercent -BaselineOrder $baselineOrder -CandidateOrder $candidateOrder
    ...
    $entry = [pscustomobject]@{
        ...
        hitches                = "$($a.hitch16_67)/$($a.hitch33_33)/$($a.hitch50) -> $($b.hitch16_67)/$($b.hitch33_33)/$($b.hitch50)"
        drift                  = $drift
