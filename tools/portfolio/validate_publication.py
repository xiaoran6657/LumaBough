...
# 例外只针对**明确的 Python 测试文件**：它们只验证工具与记录，不参与运行包构建。
# 同目录的 .cpp/.h/CMakeLists.txt 仍是真正的构建输入（会被编译成测试目标），不能一并豁免。
BUILD_EXEMPT_PY = re.compile(r"^tests/tools/.+\.py$")
...
def build_sensitive(path):
    """该路径是否属于"改了就必须重建"的构建输入（tests/tools 下的 Python 测试文件不算）。"""
    if BUILD_EXEMPT_PY.match(path):
        return False
