...
状态：**候选通过（下列阻断项收口后复跑的真实输出）**；`publication=BLOCKED`（无 remote、未推送、未公开）。

## 当前阻断与收口记录（先如实记录，再写 PASS）

| 阻断项 | 处置 | 结果 |
| --- | --- | --- |
| 构建敏感输入把 `tests/tools/` 的 Python 契约测试也判成构建输入 | `build_sensitive()` 精确排除 `tests/tools/`，保留 `tests/` 下的 C++ 目标；打包器改为直接调用同一入口（不再自行组合常量），并加正负例测试 | 已修正；无需重建 v8、无需重跑第二台机器 |
| 8 个新增处置键待批准（提案中的所有者账号名 1 个、脱敏测试路径字面量 7 个） | 按所有者书面建议批准保留（`approvedBy=owner`） | 已批准；扫描 0 待批准 |
| 文件数过期（曾写 768/769） | 更新为 771 个受审文件 + 清单自身 = 772 个 Git 文件 | 已更正 |

收口后复跑的真实输出（退出码 0）：

~~~text
PASS candidate: 771 reviewed files, package bytes, privacy gate and E4 record verified; publication=BLOCKED
NOTICE: E4 reader validation was deferred by the owner; the publication must not claim reader validation
~~~

**PASS 只在该复跑成功后写入**；在此之前本文件曾标注"候选通过"属提前宣称，已按审查意见更正。

## 候选绑定

| 项 | 值 |
| --- | --- |
| 源码集合 | `PUBLICATION-FILES.json`：771 个受审文件；含清单自身共 772 个 Git 文件 |
...
| 性能证据包 | 56 文件 / 1,082,886 B / ZIP `c33c5b39d05b77b5587acdfe85b9131e377aeb48828c61ebe21acc0c10489ff5` |
| 聚合外部校验 | `out/e5/attachments/SHA256SUMS-external.txt`（覆盖上面 4 个附件自身哈希） |
...
3. **EXE 与构建绑定**：包内 EXE 与 `exeSha256` 一致；`builtAtCommit` 是 HEAD 的祖先；
   `build_sensitive()` 判定的构建输入（含 `tools/benchmark/`、`tools/assets/`、`tools/CMakeLists.txt`，
   排除 `tests/tools/`）在该提交之后没有改动。
4. **E3 隐私门**：`source-set` + `git` + 包 ZIP 三目标重扫，**130 项 finding、0 未裁定、0 待批准、57 个键全部 approved，退出 0**；
   二进制字符串另行核查：EXE 内**不含**工程绝对路径。
5. **E4 记录**：六条必需运行 ID 齐全、不重复、`complete=true`、正例 0 / 负例 2，且每条绑定本包 EXE；
   读者走查按所有者决定延后（门禁打印提示，不静默）。
6. **依赖与许可**：[依赖缺口](DEPENDENCY-GAPS.md)、[许可预审](LICENSING-REVIEW.md) 与包内 `licenses/`
   （自有 MIT、运行包副本的第三方/资产说明、`REDISTRIBUTABLES.md` 记录 VC 运行时 14.51.36247.0 与 WinPixEventRuntime 哈希）。
...
8. **性能证据**：[证据包](PERFORMANCE-EVIDENCE-PACK.md) 去标识逐帧输入，包内分析器复算与冻结结论逐项一致
   （1 INCONCLUSIVE + 3 REJECTED）；隐私扫描 0 项。
9. **测试**：工具测试 52 项、契约测试 59 项均 OK（含候选门字节核对、构建敏感正负例、E4 完整性负例、脱敏契约、读者状态语义）。
...
- 原始 Capture、视频原片、PDB、烘焙报告、冻结性能原始数据只留本地 `out`，不进候选集合。
