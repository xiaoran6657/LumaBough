// ============================================================================
// PbrCommon.hlsli — M4 PBR 公共函数库（02 篇「Direct-light BRDF」的 HLSL 侧唯一实现）
// 里程碑：M4（02 篇 Material 资产与 metallic-roughness PBR）
// 职责：固定 Trowbridge-Reitz GGX NDF、Smith with Schlick-GGX geometry、Schlick
//       fresnel 与 Lambert diffuse 的 direct-light BRDF。全部计算在线性空间；
//       契约与 tests/rendering/PbrMathTests.cpp 的 CPU 参考实现一一对应。
//       dielectric F0 固定 0.04；roughness 由调用方 clamp 到 MIN_ROUGHNESS 以上。
// 关联：docs/architecture/README.md「Direct-light BRDF」
//       shaders/d3d11/PbrForward.hlsl（调用方；shadow/IBL 变体沿用本文件）
// ============================================================================
#ifndef MINIENGINE_PBR_COMMON_HLSLI
#define MINIENGINE_PBR_COMMON_HLSLI
...
// f0 与 max(1-roughness, f0) 之间的插值避免完全粗糙材质在边缘出现不物理的高光。
...
// 编译错误——与退化输入返回 0 的语义等价的单出口形式在两种口径下都干净。
...
// 输入位模式固定，输出确定——序列跨设备/跨运行逐位一致（04 篇硬约束）。
...
// alpha = roughness²（Disney 重参数化）；分母下限 1e-7 防 alpha→0 退化。
